...
export type Bulk = { path: string, fullPath: string, flags: number, bulkEpoch: number, hasObb: boolean }
...
const decodeBulk = (pl: Uint8Array): Bulk[] => {
  const text = new TextDecoder().decode(pl.slice(0, 256))
  if(text.includes('error')) throw new Error(`earth err: ${text}`)
...
const cache = new Map<string, Promise<Bulk[]>>()
...
export const canDescend = (e: Bulk) => e.path.length === 4 && !(e.flags & 4)
export const hasRich3dData = (e: Bulk | undefined) => !!e && !(e.flags & 2)